...
export default function EIIconInput({ icon, placeholder, _onChange, _onDoubleClick, answer }: EIIconInputProps) {
	...
	return (
		<div className={`${inputWrapClasses} ${answer}`} onDoubleClick={_onDoubleClick}>
			{icon}
			<input className={`${inputClasses} ${answer}`} placeholder={placeholder} onChange={_onChange} />
		</div>
	);
}
